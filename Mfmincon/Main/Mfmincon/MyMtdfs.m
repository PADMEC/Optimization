...
%                 aT(end+1,:)=atx;
                %a=a-mf';
                %[xx,ffobj,Converg,Resumo]=fmincon(fun,[x(end,:) t],[],[],[],[],vlb,vub,con,options,3,mf,a,n,0,par,2);
                [xt,ffobj,Converg,Resumo]=fmincon(funmtd,[x0 t],[],[],[],[],vlb,vub,conmtd,options,fun,con,-1,mf,l,a,n',par);
                ...
                x(end+1,:)=xt;
                %x0=xt;
                f0(i,:)=feval(fun,x(end,:));
                
                options=optimset(options,'TolCon',1e-4);
                [xi,ffobj,Convrg,Resm]=fmincon('constfun',xt,[],[],[],[],vlb,vub,'constcon',options,fun,con,inv,f0(i,par),0,f0(i,:),par);
                options=optimset(options,'TolCon',1e-6);
                if norm(xi-x0)/norm(x0)>1e-4
                    %x0
                    %xi
                    x(end,:)=xi;
                    x0=xi
...
%% Sub optimization
        ...
        for i=1:size(b,1)
            ...
            %Find int point 
            ax(par)=a;
            [mnd,ix]=min(sum((a*ones(1,size(aT,1))-aT(:,par)').^2));
            x0=x(end,:);
            if norm((a'-aT(end,par))./l)>2*norm((a'-aT(ix,par))./l)
                x0=Xa(ix,:);
            end 
            ...
            %t],[],[],[],[],vlb,vub,'nbicon',options,fun,con,-1,mf,l,a,n',par);
        ...
        %     end
            ...
            if ~isempty(inv)
            options=optimset(options,'TolCon',1e-4);
                [xt,ffobj,Conver,Resum]=fmincon('constfun',xt,[],[],[],[],vlb,vub,'constcon',options,fun,con,inv,fcurnt(:,par),0,fcurnt,par);
            options=optimset(options,'TolCon',1e-6);
            end
